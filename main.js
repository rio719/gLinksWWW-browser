const { app, autoUpdater,globalShortcut, clipboard, BrowserView, BrowserWindow, ipcMain, powerSaveBlocker } = require('electron');
const path = require('path');
const fs = require('fs');
// 영상이 로딩될 때 OS가 프로세스를 잠재우지 못하도록 차단합니다.
// 2. 하드웨어 가속 명시적 비활성화 (Electron 9+ 권장)
app.disableHardwareAcceleration();  // GPU 프로세스 완전 차단

// 3. 소프트웨어 렌더링 + 유튜브 최적화 스위치
app.commandLine.appendSwitch('disable-software-rasterizer');
app.commandLine.appendSwitch('disable-gpu-compositing');  // GPU 컴포지팅 off
app.commandLine.appendSwitch('ignore-gpu-blocklist');    // GPU 블랙리스트 무시
...
app.commandLine.appendSwitch('disable-ipc-flooding-protection');  // 이전 쿼리 관련
let win;
let views = []; 
    ...
    for (let i = 0; i < 10; i++) {
        globalShortcut.register(`CommandOrControl+Shift+${i}`, () => {
            if (views[currentTabIndex]) {
                ...
                setTimeout(() => {
                    multiClipboard[i] = clipboard.readText();
                    fs.writeFileSync(storagePath, JSON.stringify(multiClipboard));
                }, 100);
            }
        });
        globalShortcut.register(`CommandOrControl+Alt+${i}`, () => {
            if (multiClipboard[i] && views[currentTabIndex]) {
                clipboard.writeText(multiClipboard[i]);
