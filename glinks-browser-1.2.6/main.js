const { app,session, autoUpdater,globalShortcut, clipboard, BrowserView, BrowserWindow, ipcMain, powerSaveBlocker, dialog } = require('electron');
const path = require('path');
const fs = require('fs');
// 영상이 로딩될 때 OS가 프로세스를 잠재우지 못하도록 차단합니다.
// 2. 하드웨어 가속 명시적 비활성화 (Electron 9+ 권장)
// 1. 하드웨어 가속 비활성화 (저사양 PC 및 권한 충돌 방지)
app.disableHardwareAcceleration(); 

// 2. 백그라운드 작업 보장 (다중 복붙 로직이 잠들지 않게 함)
// 이 스위치들이 없으면 앱이 최소화될 때 클립보드 저장 로직이 멈출 수 있습니다.
...
app.commandLine.appendSwitch('disable-renderer-backgrounding');
app.commandLine.appendSwitch('disable-backgrounding-occluded-windows');
// 4. 단일 인스턴스 고정 (중복 실행으로 인한 권한 꼬임 방지)
...
function createTabView(index) {
    const v = new BrowserView({
        webPreferences: { 
            ...
            contextIsolation: false, // 리오님 설정 유지
            // 👇 이 경로 설정을 더 명확하게 바꿔보세요
